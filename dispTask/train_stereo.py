...
def main():
    ...
    #=============================== Train routine =====================================
    #===================================================================================
    foldercheck(save_path+'/psm'); foldercheck(save_path+'/sfcn')
    for epoch in range(args.start_epoch, args.epochs): # epochs chosen to be 3e6 because we want to end training based on iterations - 300k
        # train for one epoch
        train_avg_slic, train_avg_sem, iteration = train(train_loader, (model, psmnet), optimizer, epoch,
        ...
        with torch.no_grad():
            avg_slic, avg_psm  = validate(val_loader, (model, psmnet), epoch, val_writer, val_spixelID, val_XY_feat_stack)
            if epoch % args.record_freq == 0:
        ...
        psm_dict = {
                'epoch': epoch + 1,
                'arch': args.arch,
                'state_dict': psmnet.module.state_dict(),
                'best_EPE': best_EPE,
                'optimizer': optimizer.state_dict(),
                'dataset': args.dataset
            }
        ...
        if (iteration) >= (args.milestones[-1] + args.additional_step):
            save_checkpoint(psm_dict, is_best =False, save_path=f'{save_path}/psm', filename='%d_step.tar' % iteration)
            save_checkpoint(rec_dict, is_best =False, save_path=f'{save_path}/sfcn', filename='%d_step.tar' % iteration)
            print("Train finished!")
            break

        if best_EPE < 0:
            best_EPE = avg_psm
        is_best = avg_psm < best_EPE
        best_EPE = min(avg_psm, best_EPE)
        save_checkpoint(rec_dict, is_best, save_path=f'{save_path}/sfcn')
        save_checkpoint(psm_dict, is_best, save_path=f'{save_path}/psm' )
...
def train(train_loader, networks, optimizer, epoch, train_writer, init_spixl_map_idx, XY_feat_stack):
    ...
    for i, sample in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            print('train Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_slic {6}\t Loss_psm {7}\t'
...
def validate(val_loader, networks, epoch, val_writer, init_spixl_map_idx, XY_feat_stack):
    ...
    with torch.no_grad():
        for i, sample in enumerate(val_loader):
            ...
            if i % args.print_freq == 0:
                print('val Epoch: [{0}][{1}/{2}]\t Time {3}\t Data {4}\t Total_loss {5}\t Loss_slic {6}\t Loss_psm {7}\t'
                    .format(epoch, i, epoch_size, batch_time, data_time, total_loss, losses_slic, losses_psm))
...
def save_checkpoint(state, is_best, save_path, filename='checkpoint.tar'):
