...
# ================ training setting ====================
parser.add_argument('--dataset', metavar='DATASET', default='NYU',  choices=dataset_names,
...
parser.add_argument('--data', metavar='DIR',default='./NYU', help='path to input dataset')
parser.add_argument('--savepath',default='checkpoints', help='path to save ckpt')
...
def train(train_loader, model, optimizer, epoch, train_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, depth) in enumerate(train_loader):
...
def validate(val_loader, model, epoch, val_writer, init_spixl_map_idx, xy_feat):
    ...
    for i, (input, label, depth) in enumerate(val_loader):
