...
class Compose(object):
    ...
    def __call__(self, input, label, depth):
        for t in self.co_transforms:
            input,label, depth = t(input,label, depth)
        return input,label, depth
...
#     def __call__(self, inputs, target, depth):
...
class Resize(object):
    ...
    def __init__(self, size= (256, 512)):
        if isinstance(size, numbers.Number):
            ...
            self.size = size
    def __call__(self, inputs,targets, depth):
        for i in range(len(inputs)):
            inputs[i] = cv2.resize(inputs[i], (self.size[1],self.size[0]) )
            targets[i] = cv2.resize(targets[i], (self.size[1],self.size[0]))[..., np.newaxis]
        depth = cv2.resize(depth, (self.size[1],self.size[0]) )[..., np.newaxis]
        return inputs, targets, depth
...
class CenterCrop(object):
    ...
    def __call__(self, inputs,targets, depth):
        ...
        return inputs, targets, depth[y1: y1 + th,x1: x1 + tw]
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, inputs, targets, disp) :
        if random.random() < 0.5:
            for i in range(len(inputs)):
