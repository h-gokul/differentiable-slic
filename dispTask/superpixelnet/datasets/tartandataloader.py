...
class TartanStereoLoader(Dataset):
    def __init__(self, basepath, mode = 'train', scale = 2):
        self.transform=None
    ...
    def loadfiles(self, path):
        ...
        for folder in iterator:
            self.Lfiles = sorted(glob.glob(path+'/'+folder+'/image_left/*png'))
            self.Rfiles = sorted(glob.glob(path+'/'+folder+'/image_right/*png'))
            self.Ldepthfiles = sorted(glob.glob(path+'/'+folder+'/depth_left/*depth.npy'))
            self.Rsegfiles = sorted(glob.glob(path+'/'+folder+'/seg_right/*seg.npy'))
            self.Lsegfiles = sorted(glob.glob(path+'/'+folder+'/seg_left/*seg.npy'))
            # poselist = np.loadtxt(path+'/'+folder+'/pose_left.txt').astype(np.float32)

            assert len(self.Rfiles) == len(self.Lfiles) ==len(self.Ldepthfiles) == len(self.Lsegfiles)== len(self.Rsegfiles), \
                f"{len(self.Rfiles), len(self.Lfiles), len(self.Ldepthfiles),  len(self.Lsegfiles), len(self.Rsegfiles)}"
    ...
    def __getitem__(self, idx):
        ...
        # get images
        imL = cv2.imread(self.Lfiles[idx])
        imR = cv2.imread(self.Rfiles[idx])
        dispL = 80.0/np.load(self.Ldepthfiles[idx])[..., np.newaxis]
        segL = np.load(self.Lsegfiles[idx])[..., np.newaxis]
        segR = np.load(self.Rsegfiles[idx])[..., np.newaxis]
        
        # crop and resize
        resized_items = []
        for x in [imL, imR, dispL, segL, segR]:
            x = self.downsize(crop64(x)).transpose(2,0,1)            
            resized_items.append(x)
        imL, imR, dispL, segL, segR = resized_items 

        dispL[dispL<80./100.0]=0

        # nflow = torch.from_numpy(flow) # uncomment if you wanna pass optical flow
        return imL, imR, dispL,  segL, segR
...
class TartanOdomLoader(Dataset):
