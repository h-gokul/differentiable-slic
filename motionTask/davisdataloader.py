...
class DAVISSegmentationLoader(Dataset):
    ...
    def __init__(self, basepath="./Data/DAVIS", mode = "val", shape = (256,256)):
        if mode == 'val':
            folders = sorted(os.listdir(f"{basepath}/JPEGImages/480p"))[1:10]
        elif mode =='train':    
            folders = sorted(os.listdir(f"{basepath}/JPEGImages/480p"))[10:]
        else: 
            folders= [mode] # test scenarios - directly with folder
