...
def superpixel_seg( seg, spix_indices):
    B, _, H, W  = spix_indices.size()
    spix_indices = spix_indices.reshape(B,1, -1)
    seg = seg.reshape(B,1, -1)

    for b in range(seg.size(0)):
        for Ci in range(len(torch.unique(spix_indices))):
            Ci_ROI = spix_indices == Ci
            segCi_patch = seg[b, :1][Ci_ROI[b]] 
            segmedian = torch.median(segCi_patch)
            # print(meanx.item(), meany.item())
            seg[b][Ci_ROI[b]] = segmedian
    return seg.reshape(B, 1, H, W)
