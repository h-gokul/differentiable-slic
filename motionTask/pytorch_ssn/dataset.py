...
from model.util import convert_index
import cv2
...
class Resize(object):
    """
    Resize flow, images, depth etc
    """
    def __init__(self, shape):
        self.new_h, self.new_w = shape
    def __call__(self, sample, shape):
        h,w = shape 
        self.downscaley, self.downscalex = self.new_h/h , self.new_w/w
         
        if self.downscalex !=1.0 and self.downscaley !=1.0:
            if len(sample.shape)==3:
                # flow : 
                if sample.shape[2] == 2:
                        flow = cv2.resize(sample, (0, 0), fx=self.downscalex, fy=self.downscaley, interpolation=cv2.INTER_LINEAR)
                        flow[:, :, 0] = flow[:, :, 0] * self.downscalex
                        flow[:, :, 1] = flow[:, :, 1] * self.downscaley
                        return flow       
                # image:
                elif sample.shape[2] == 3:
                    return cv2.resize(sample, (0, 0), fx=self.downscalex, fy=self.downscaley, interpolation=cv2.INTER_LINEAR)                         
            else:                
                # intrinsics: 
                if sample.shape[0] == 3 :
                    sample[0] *=  self.downscalex
                    sample[1] *=  self.downscaley            
                    return sample
                # depth - nearest neighbor always for depth
                else:
                    return cv2.resize(sample, (0, 0), fx=self.downscalex, fy=self.downscaley, interpolation=cv2.INTER_NEAREST)
        else:
            return sample
...
#         # padding the image for suitable h,w shape
#         out_img = transform_and_get_image(im_cropped, self.num_spixel, [out_height, out_width])
#         # add xy information to make it as rgbxy - 5 channels
...
#         self.img_dir = os.path.join(self.data_dir, 'images', self.dtype)
#         self.split_list = os.listdir(self.img_dir)[:10]
#         self.gt_dir = os.path.join(self.data_dir, 'groundTruth', self.dtype)

#     def __getitem__(self, item):
#         img_name = self.split_list[item].rstrip('.jpg\n')
#         image = img_as_float(io.imread(os.path.join(self.img_dir, img_name + '.jpg')))
