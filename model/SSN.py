...
class create_ssn_net(nn.Module):
    ...
    def forward(self, x, args):
        p2sp_index, invisible, init_index, cir_index, problabel, spixel_h, spixel_w, device = args
...
def SSN(pretrained, spixel_size, num_steps=10, dtype = 'layer', device = 'cuda'):    
    ...
    model = create_ssn_net(num_spixels=num_spixels, num_iter=num_steps, num_spixels_h=num_spixel_h, num_spixels_w=num_spixel_w, dtype=dtype, ssn=bool(pretrained)).to(device)
